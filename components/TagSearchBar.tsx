...
import { autocomplete } from "@/lib/fetcher";
...
const Highlight = ({ label, q }: { label: string; q: string }) => {
  ...
  return (
    <>
      {label.slice(0, i)}
      <mark className="rounded-sm bg-amber-500/40 px-0.5 text-amber-100">{label.slice(i, i + q.length)}</mark>
...
const TagBadge = ({ tag, onRemove }: { tag: string; onRemove: () => void }) => (
  <span className="inline-flex items-center gap-0.5 rounded-full bg-zinc-800 pl-2.5 pr-1 py-0.5 text-xs text-zinc-200">
    {tag}
    <button
      type="button"
  ...
  return (
    <div className={compact ? "relative w-full" : "relative w-full max-w-md"}>
      ...
      <div className="relative">
        <span
          className="pointer-events-none absolute inset-y-0 left-0 flex w-10 items-center justify-center text-zinc-500"
          aria-hidden
        >
          <Search size={16} strokeWidth={2} />
        </span>
        <input
          className={`w-full rounded-xl border border-zinc-800 bg-zinc-900 text-sm text-zinc-100 placeholder:text-zinc-500 outline-none focus:border-zinc-600 ${
      ...
      {open && (
        <ul className="absolute top-full right-0 left-0 z-30 mt-1 max-h-48 overflow-y-auto rounded-xl border border-zinc-700 bg-zinc-900 text-zinc-200 shadow-xl shadow-black/50">
      ...
      {!compact && (
        <button
          ...
          className="mt-4 w-full rounded-xl border border-zinc-600 bg-zinc-800 py-2.5 text-sm font-semibold text-zinc-100 hover:bg-zinc-700 disabled:opacity-40"
          onClick={submit}
        >
